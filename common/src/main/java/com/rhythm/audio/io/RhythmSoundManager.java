...
public class RhythmSoundManager {
    ...
    private static final ConcurrentHashMap<String, Consumer<Boolean>> completionListeners = new ConcurrentHashMap<>();

    private record PendingSound(String fileName, String url, boolean loop) {}

    // ==================== Completion Listener API ====================

    /**
     * Registers a listener to be notified when a download completes.
     * The listener receives true on success, false on failure.
     * The listener is automatically removed after being called.
     *
     * @param fileName the file name to listen for
     * @param listener callback receiving success/failure status
     */
    public static void addCompletionListener(String fileName, Consumer<Boolean> listener) {
        completionListeners.put(fileName, listener);
    }

    /**
     * Removes a completion listener without triggering it.
     *
     * @param fileName the file name to stop listening for
     */
    public static void removeCompletionListener(String fileName) {
        completionListeners.remove(fileName);
    }

    /**
     * Checks if a download is currently in progress for the given file.
     *
     * @param fileName the file name to check
     * @return true if download is in progress
     */
    public static boolean isDownloading(String fileName) {
        return RhythmExecutable.YT_DLP.isProcessRunning(fileName + DOWNLOAD_PROCESS_SUFFIX);
    }
    ...
    private static void handleDownloadError(String fileName) {
        ...
        notifyCompletionListener(fileName, false);
    }
    ...
    private static void handleDownloadComplete(String fileName) {
        ...
        notifyCompletionListener(fileName, true);
    }

    private static void notifyCompletionListener(String fileName, boolean success) {
        Consumer<Boolean> listener = completionListeners.remove(fileName);
        if (listener != null) {
            try {
                listener.accept(success);
            } catch (Exception e) {
                RhythmConstants.LOGGER.error("Error in download completion listener for {}: {}", fileName, e.getMessage());
            }
        }
    }
    ...
    public static String getFileName(String url) {
        return RhythmAudioCache.getFileHash(url);
    }
}
